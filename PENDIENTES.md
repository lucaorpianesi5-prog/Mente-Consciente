...
- [ ] Estado global disperso sin encapsulamiento (variables `let` a nivel de script) — auditoría del 09/07 (MANTENIBILIDAD, refactor grande) — decisión de Luca: no tocar. Arreglarlo de verdad implica modificar casi todas las funciones del archivo sin ningún test automático que confirme que no se rompió nada; el beneficio es solo prolijidad interna, sin bug real asociado. Reconsiderar solo si se suma otro programador al proyecto o se migra a backend.
- [ ] Patrón de `onclick` inline pervasivo en vez de `addEventListener` — auditoría del 09/07 (MANTENIBILIDAD, refactor grande) — decisión de Luca: no tocar. Mismo riesgo que el estado global (cientos de puntos a cambiar sin tests), sin bug real detrás. Reconsiderar solo si en algún momento hace falta una política CSP estricta.
- [ ] Re-render completo vía `innerHTML` en cada actualización de estado (`renderCobros`, `renderNutri`, `renderFinanzas`) — auditoría del 09/07 (RENDIMIENTO/BAJO, refactor grande) — decisión de Luca: no tocar. La propia auditoría ya marcaba esto como imperceptible hoy, solo preocupación de escala futura.
...
- 2026-07-12 — Eliminada la duplicación de lógica de fechas/horarios entre app y portal (`portalCalcEdad`/`portalStringToHorarios` borradas, sus 2 únicos usos apuntan ahora a `calcularEdad`/`stringToHorariosPorDia`) — commit `067fa99`.
...
- 2026-07-20 — **"Disponible" en Finanzas** — reemplaza la columna "Ahorro" manual (tipeada mes a mes) por un cálculo automático: saldo inicial configurable + todo lo cobrado históricamente a alumnos y nutrición (`totalCobradoHistorico()`, sin filtrar por alumnos actualmente activos, a diferencia de `getIngresosMes()`) − gastos. Card fija arriba de Finanzas, no depende del año seleccionado. El tab `Ahorros` del Sheet pasa de log mensual a una sola fila de config (`SaldoInicialAhorro`/`FechaReferenciaSaldoInicial`). De paso, `sheetsGet()` ganó un parámetro `unformatted` para pedir valores sin formatear de la API (evita que `parseFloat` rompa con separador de miles tipo "150.000").
- 2026-07-20 — **Auditoría total retroactiva pre-features** (handoff `handoff-auditoria-total.md`, chat de Auditoría Opus 4.8). Resultado: (1) **Reconstrucción de `b5d8ce8` verificada completa** — los 8 bloques de trabajo no auditados (allowlist, escritura por fila, columna Id de Seguimiento, borrado de eventos al pausar + paginación, dedup portal, Biblioteca/Progresión, Disponible) están presentes y coherentes; sin código perdido ni a medio aplicar. (2) **Proxy `portal-proxy.gs`: SEGURO** — token `crypto.randomUUID()` (122 bits, no forzable, hace irrelevante la falta de rate limiting); superficie de datos correctamente acotada al alumno del token (no filtra teléfono/notas/monto/historialFis/ID real/datos de otros); `Logger.log(err)` de riesgo bajo (visible solo para el dueño). **Sí es seguro sumar recordatorios automáticos, con una condición**: implementarlos como *time-driven trigger* aparte (corre como Luca, con acceso a teléfonos), NUNCA como rama nueva del `doGet` público —`doGet` debe seguir siendo solo-lectura y acotado al token. Esto resuelve el ítem "Verificación de seguridad del proxy" de Decisiones pendientes. (3) Dos hallazgos LOW nuevos anotados en Bugs activos (`historialFis` sin escapar, `fechaReferencia` cosmética). (4) Se confirmó que Biblioteca/Progresión NO inclina la balanza para reabrir las decisiones aceptadas de estado global / `onclick` inline (incremento marginal, no salto de escala).
- 2026-07-20 — **Incidente de reconciliación de Git**: un upload manual a `main` (patrón ya conocido, ver memoria de proyecto) partió de una copia vieja de `index.html` y sin querer revirtió 4 commits de trabajo ya confirmado (allowlist de login, escritura por fila en Cobros/Gastos/Nutrición/Seguimiento, columna `Id` de Seguimiento, paginación de borrado de eventos de Calendar, Biblioteca/Progresión). Se detectó antes de que nadie lo notara en producción y se reconstruyó el archivo partiendo de la última versión buena (`b6f89dc`) reaplicando solo el trabajo nuevo de Disponible. Sin código perdido.
- 2026-07-20 — Los 2 hallazgos LOW de la auditoría del 20/07, cerrados: (1) `historialFis` ahora pasa por `escaparHTML()` en el textarea del perfil, igual que el resto del archivo — self-XSS corregido, sin cambio de comportamiento en el guardado (el browser desescapa al leer `.value`, round-trip intacto). (2) `fechaReferencia` del saldo inicial: decisión de Luca fue **Opción B** — el Sheet de Cobros nunca va a tener pagos de meses anteriores a cuando se fija el saldo inicial, así que no hay riesgo real de doble conteo. Se sacó el input de fecha del formulario de edición y la etiqueta ahora dice fijo "al empezar a usar la app, antes del primer cobro registrado"; cero cambio en `calcularDisponible()`/`totalCobradoHistorico()`.
